...
|---|---|---|
| Component | `PascalCase.tsx` | `TourCard.tsx` |
| Page | `folder/page.tsx` | `tours/page.tsx` |
| Hook | `use<Name>.ts` | `useAuth.ts` |
| Service | `<domain>.service.ts` | `tour.service.ts` |
| Types | `<domain>.types.ts` | `tour.types.ts` |
| Redux slice | `<domain>Slice.ts` | `authSlice.ts` |
| Server Action | `<domain>.actions.ts` | `tour.actions.ts` |
| Page exports | `default export` | Required by Next.js |
| Everything else | Named exports | `export const TourCard` |
...
export const USER_ROLES = { USER: 'USER', COMPANY: 'COMPANY', ADMIN: 'ADMIN', GUIDE: 'GUIDE', DRIVER: 'DRIVER' } as const;
...
Protected paths: `/dashboard`, `/profile`, `/my-tours`, `/admin` — redirect to `/login` if no token.
