...
- `code`: stable machine-readable string (e.g. `TOUR_NOT_FOUND`, `VALIDATION_FAILED`)
...
// Allowed
return reply.send(paginatedResponse("Tours retrieved successfully", tours, page, limit, totalCount));
```
...
```typescript
// Service returns:
return { items: tours, totalItems: count };
...
| `NotFoundError` | 404 | `TOUR_NOT_FOUND` |
