...
export function BoughtTogether({ mainProduct, relatedProducts, locale }: BoughtTogetherProps) {
  const t = useTranslations();
  const addItem = useCartStore((s) => s.addItem);

  // Track which related products are selected (all on by default)
  ...
  // Total = main product + selected related
      ...
      <div className="flex items-center justify-between gap-4 px-5 py-4 border-t border-border bg-muted/30">
        <div className="flex flex-col gap-0.5">
          <span className="text-[11px] text-muted-foreground">
            {t('catalog.bought_together_total')} ({allProducts.length} {t('catalog.bought_together_items')})
          </span>
        ...
        <button
          ...
          aria-label={t('catalog.bought_together_buy_all')}
        >
          {added ? (
            ...
            <>
              ...
              <span>{t('catalog.bought_together_buy_all')}</span>
            </>
          )}
        </button>
      </div>
...
function BoughtTogetherItem({ product, locale, isMain, isSelected, onToggle }: BoughtTogetherItemProps) {
