            ...
            <div key={cat.id}>
              <div className="flex items-center gap-2 px-4 py-2 hover:bg-muted/50 transition-colors">
                <div className="flex flex-col shrink-0">
                  <Button type="button" variant="ghost" size="icon-xs" onClick={() => moveCategory(catIdx, -1)} aria-label="Move up" className="text-muted-foreground hover:text-foreground">
                  ...
                  <Button type="button" variant="ghost" size="icon-xs" onClick={() => moveCategory(catIdx, 1)} aria-label="Move down" className="text-muted-foreground hover:text-foreground">
                ...
                <Button type="button" variant="ghost" size="icon-xs" onClick={() => toggleExpanded(cat.id)} aria-label="Toggle details" className="text-muted-foreground">
                ...
                <span className="text-sm text-foreground font-medium flex-1 truncate">{cat.label.en || cat.label.ka || '(unnamed)'}</span>
                {childCount > 0 && (
                  <span className="text-[10px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded-full">{childCount} sub</span>
                )}

                {cat.id !== 'all' && (
                  <div className="flex items-center gap-0.5 shrink-0">
                    <Button type="button" variant="ghost" size="icon-xs" onClick={() => addSubcategory(catIdx)} aria-label="Add subcategory" title="Add sub" className="text-muted-foreground hover:text-foreground">
                    ...
                    <Button type="button" variant="ghost" size="icon-xs" onClick={() => removeCategory(catIdx)} aria-label="Delete category" className="text-muted-foreground hover:text-destructive">
                    ...
                    <div className="space-y-1 mt-2">
                      ...
                      {cat.children.map((child, childIdx) => (
                        <div key={child.id} className="grid grid-cols-6 gap-1.5 items-center py-1">
                          ...
                          <Input value={child.specFilter?.kaKey ?? ''} onChange={(e) => updateSubcategory(catIdx, childIdx, { ...child, specFilter: { kaKey: e.target.value, value: child.specFilter?.value ?? '' } })} placeholder="Spec key" />
                          <Input value={child.specFilter?.value ?? ''} onChange={(e) => updateSubcategory(catIdx, childIdx, { ...child, specFilter: { kaKey: child.specFilter?.kaKey ?? '', value: e.target.value } })} placeholder="Value" />
                          <Button type="button" variant="ghost" size="icon-xs" onClick={() => removeSubcategory(catIdx, childIdx)} className="text-muted-foreground hover:text-destructive justify-self-start" aria-label="Remove">
