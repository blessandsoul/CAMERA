...
export function CategoryNavBar({ locale, counts }: CategoryNavBarProps): React.ReactElement {
  return (
    <nav
      aria-label="Product categories"
      className="flex items-stretch justify-center gap-1.5 sm:gap-2 rounded-2xl border border-border/50 bg-card/80 backdrop-blur-sm p-2 sm:p-2.5 shadow-sm"
    >
      {CATEGORIES.map((cat) => {
        ...
        return (
          ...
          >
            <div className="flex items-center justify-center w-10 h-10 sm:w-11 sm:h-11 rounded-xl bg-primary/8 border border-primary/10 text-primary/70 group-hover:bg-primary/12 group-hover:text-primary group-hover:border-primary/20 transition-all duration-200">
              {cat.icon}
            </div>
            ...
            {count !== undefined && count > 0 && (
              <span className="absolute top-1.5 right-1 sm:top-2 sm:right-2 text-[9px] font-bold tabular-nums text-primary bg-primary/10 border border-primary/15 px-1.5 py-px rounded-full leading-none">
                {count}
              </span>
            )}
          </Link>
