---
id: camera-008
slug: uniview-2-outdoor-camera-4
category: cameras
price: 79
currency: GEL
isActive: true
isFeatured: true
images:
  - "Untitled-1_0zo4-6e.jpg"
name:
  ka: 2MP HD IR Fixed Mini Bullet Network Camera 4.0, Uniview - IPC2122LB-SF40K-A
  ru: Uniview 2 მპ уличная камера 4 მმ
  en: Uniview 2 მპ outdoor Camera 4 მმ
specs:
  - key: { ka: "ბრენდი", en: "Brand", ru: "Бренд" }
    value: "Uniview"
  - key: { ka: "კამერის ტიპი", en: "Camera Type", ru: "Тип камеры" }
    value: "IP"
  - key: { ka: "კორპუსის ტიპი", en: "Body Type", ru: "Тип корпуса" }
    value: "გარე გამოყენების"
  - key: { ka: "CMOS სენსორი", en: "CMOS Sensor", ru: "CMOS сенсор" }
    value: "1/2.9\" CMOS"
  - key: { ka: "ლინზა", en: "Lens", ru: "Объектив" }
    value: "4 მმ"
  - key: { ka: "ხედვის კუთხე", en: "View Angle", ru: "Угол обзора" }
    value: "83.7°"
  - key: { ka: "ღამის ხედვა", en: "Night Vision", ru: "Ночное видение" }
    value: "ჭკვიანი"
  - key: { ka: "ღამის ხედვა (მანძილი)", en: "Night Vision Range", ru: "Дальность ночного видения" }
    value: "30 მეტრი"
  - key: { ka: "რეზოლუცია", en: "Resolution", ru: "Разрешение" }
    value: "2 მპ"
  - key: { ka: "Micro SD", en: "Micro SD", ru: "Micro SD" }
    value: "128 GB"
  - key: { ka: "დაცვის კლასი", en: "Protection Class", ru: "Класс защиты" }
    value: "IP 67"
createdAt: "2026-02-18T13:55:02.851Z"
---

2MP HD IR Fixed Mini Bullet Network Camera 4.0, Uniview - IPC2122LB-SF40K-A. გარე გამოყენების გამოყენება. რეზოლუცია: 2 მპ. ლინზა: 4 მმ.
